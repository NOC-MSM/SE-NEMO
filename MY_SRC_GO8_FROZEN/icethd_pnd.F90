MODULE icethd_pnd 
   ...
   REAL(wp), PARAMETER :: &   ! shared parameters for topographic melt ponds
      ...
      zTd     = 273.0_wp      , & ! temperature difference for freeze-up (C)
...
CONTAINS
   ...
   SUBROUTINE pnd_LEV
      ...
      !!
      !! ** References :   Holland et al (J. Clim, 2012), Hunke et al (OM 2012)
      !!                   
      !!-------------------------------------------------------------------
         ...
         DO jl = 1, jpl
            ...
            DO ji = 1, npti
               ...
               ELSE                                                         ! Case ice thickness >= rn_himin !
                  ...
                  diag_dvpn_mlt_1d(ji) = diag_dvpn_mlt_1d(ji) + zdum * r1_rdtice                      ! surface melt input diag
                  diag_dvpn_rnf_1d(ji) = diag_dvpn_rnf_1d(ji) + ( 1. - zfr_mlt ) * zdum * r1_rdtice   ! runoff diag
                  !
                  !--- overflow ---!
                  !
                  ...
                  zdv_mlt = MAX( 0._wp, MIN( zdv_mlt, zv_ip_max - v_ip_1d(ji) ) )
                  diag_dvpn_rnf_1d(ji) = diag_dvpn_rnf_1d(ji) + ( zdv_mlt - zvold ) * r1_rdtice       ! runoff diag - overflow contribution
                  ...
                  diag_dvpn_lid_1d(ji) = diag_dvpn_lid_1d(ji) + ( v_ip_1d(ji) - zvold ) * r1_rdtice   ! shrinking counted as lid diagnostic
                  ...
                  zdv_flush   = - zperm * rau0 * grav * zhp * rdt_ice / (zvisc * h_i_1d(ji)) * a_ip_1d(ji) * rn_pnd_flush
                  zdv_flush   = MAX( zdv_flush, -v_ip_1d(ji) )
                  ! zdv_flush   = 0._wp ! MV remove pond drainage for now
                  v_ip_1d(ji) = v_ip_1d(ji) + zdv_flush
                  
                  diag_dvpn_drn_1d(ji) = diag_dvpn_drn_1d(ji) + zdv_flush * r1_rdtice   ! shrinking counted as lid diagnostic
         ...
         DO jj = 1, jpj
            DO ji = 1, jpi
               ...
               IF ( a_i(ji,jj,jl) > epsi10 ) THEN
                  ...
                  diag_dvpn_mlt(ji,jj) = diag_dvpn_mlt(ji,jj) + zv_mlt * r1_rdtice                     ! diags
                  diag_dvpn_rnf(ji,jj) = diag_dvpn_rnf(ji,jj) + ( 1. - zfr_mlt ) * zv_mlt * r1_rdtice   
...
!                 zfpond(ji,jj) = zfpond(ji,jj) + zpond * a_ip_frac(ji,jj,jl) ! useless for now
      ...
      IF( ln_pnd_lids ) THEN
         ...
         DO jj = 1, jpj
            ...
            IF ( at_i(ji,jj) > 0.01 .AND. hm_i(ji,jj) > zhi_min .AND. zvolp_ini(ji,jj) > zvp_min * at_i(ji,jj) ) THEN
...
!                             zfpond(ji,jj)    = zfpond (ji,jj)    + v_il(ji,jj,jl) 
                     ...
                     ELSE IF ( v_ip(ji,jj,jl) > epsi10 ) THEN ! Tsfcn(i,j,n) <= Tp
                        ...
                        ! zdTice = MAX( - t_su(ji,jj,jl) - zTd , 0._wp ) ! > 0  ! line valid for temp in C 
