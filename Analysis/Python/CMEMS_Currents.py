...
for ilme in lmelist:    
        ...
        j,i,_=nemo_t.find_j_i_list(lat=[x_min,x_max,x_max,x_min],lon=[y_min,y_min,y_max,y_max])
        imin=min(i)
        imax=max(i)
        jmin=min(j)
        jmax=max(j)        
...
#        imax=1130
        ...
        circulation.plot_surface_circulation(nemo_t1, nemo_t1,nemo_t1, mask,'CMEMS '+LMENAM, co_located=True,Vmax=.16)
