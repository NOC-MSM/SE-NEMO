MODULE sbcssr
   ...
   !jth
   LOGICAL         ::   ln_sssr_ice_so_limit     ! limit under ice relaxation enhancement to southern ocean
   REAL(wp)        ::   rn_lat_SO_limit
...
CONTAINS
            ...
            IF( nn_sssr /= 0 .AND. nn_sssr_ice /= 1 ) THEN
              ...
              ! n.b. coefice is initialised and fixed to 1._wp if nn_sssr_ice = 1
               DO jj = 1, jpj
                  ...
                  END DO
               END DO
               !jth option to limit coefice to Southern Ocean
                IF (ln_sssr_ice_so_limit ) THEN
                   WHERE (gphit(:,:) >= rn_lat_SO_limit)
                    coefice(:,:) = 1._wp
                   END WHERE
                ENDIF
   ...
   SUBROUTINE sbc_ssr_init
      ...
      NAMELIST/namsbc_ssr/ cn_dir, nn_sstr, nn_sssr, rn_dqdt, rn_deds, sn_sst, &
              & sn_sss, ln_sssr_bnd, rn_sssr_bnd, nn_sssr_ice, ln_sssr_ice_so_limit,rn_lat_SO_limit
      INTEGER     ::  ios
      !!----------------------------------------------------------------------
      !
      IF(lwp) THEN
