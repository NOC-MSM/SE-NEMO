MODULE zdfgls
   ...
   INTEGER  ::   nn_z0_met_i       ! Method for surface roughness computation under ice
   ...
   REAL(wp) ::   rn_hsro_i         ! Surface roughness under ice in constant case nn_z0_met_i = 1
...
!!gm         zdep(:,:)  = 30._wp * TANH(  zcof/ SQRT( MAX(ustar2_surf(:,:),rsmall) )  )       ! Wave age (eq. 10)
      ...
      SELECT CASE( nn_z0_met_i )             ! Type of scaling under sea-ice
         !
         CASE( 1 )                           ! scaling with constant sea-ice thickness rn_hsro_i defined in namelist 
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
                  zhsro(ji,jj) =  ( ( 1. - fr_i(ji,jj) ) * zhsro(ji,jj) + fr_i(ji,jj) * rn_hsro_i ) * tmask(ji,jj,1) + &
                  &  + (1._wp - tmask(:,:,1)) * rn_hsro
               END DO
            END DO
            !
         CASE( 2 )                                 ! scaling with mean sea-ice thickness
            DO jj = 2, jpjm1
               DO ji = fs_2, fs_jpim1   ! vector opt.
#if defined key_si3
                  zhsro(ji,jj) = ( ( 1. - fr_i(ji,jj) ) * zhsro(ji,jj) + fr_i(ji,jj) * hm_i(ji,jj) * 2. / vkarmn ) * tmask(ji,jj,1) + &
                  &  + (1._wp - tmask(:,:,1)) * rn_hsro
#elif defined key_cice
                  zmaxice = MAXVAL( h_i(ji,jj,:) )
                  zhsro(ji,jj) = ( ( 1. - fr_i(ji,jj) ) * zhsro(ji,jj) + fr_i(ji,jj) * zmaxice / vkarmn ) * tmask(ji,jj,1) + &
                  &  + (1._wp - tmask(:,:,1)) * rn_hsro  
#endif
               END DO
            END DO
...
!!gm so, totally new staff needed!!gm
   ...
   SUBROUTINE zdf_gls_init
      ...
      NAMELIST/namzdf_gls/rn_emin, rn_epsmin, ln_length_lim,         &
         &            rn_clim_galp, ln_sigpsi, rn_hsro, rn_hsro_i,   &
         &            rn_crban, rn_charn, rn_frac_hs,                &
         &            nn_bc_surf, nn_bc_bot, nn_z0_met, nn_z0_met_i, &
