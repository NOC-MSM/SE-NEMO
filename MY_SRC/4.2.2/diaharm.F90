MODULE diaharm 
   ...
   USE daymod
   USE tide_mod
   !jth USE sbctide         ! Tidal forcing or not
   !
...
CONTAINS
   ...
   SUBROUTINE dia_harm_init 
      ...
      ENDIF
      !
          ! Namelist nam_diaharm in reference namelist : Tidal harmonic analysis
      tname(:)='un-used'
      READ  ( numnam_ref, nam_diaharm, IOSTAT = ios, ERR = 901)
901   IF( ios /= 0 )   CALL ctl_nam ( ios , 'nam_diaharm in reference namelist' )
            ! Namelist nam_diaharm in configuration namelist : Tidal harmonic analysis
...
902   IF( ios >  0 )   CALL ctl_nam ( ios , 'nam_diaharm in configuration namelist' )
      ...
      IF( ln_diaharm ) THEN
         ...
         DO jh=1,jpmax_harmo
            DO ji=1,jpmax_harmo
               IF(TRIM(tname(jh)) == tide_harmonics(ji)%cname_tide) THEN
                  nb_ana=nb_ana+1
               ENDIF
            END DO
         END DO
         ...
         IF (nb_ana > jpmax_harmo) THEN
            ...
            WRITE(ctmp2,*) ' jpmax_harmo= ', jpmax_harmo, nb_ana
         ...
         !jthCALL tide_harmo( ana_freq, vt, ut, ft, name, nb_ana )
         DO jh=1,nb_ana
            ana_freq(jh)=tide_harmonics(name(jh))%omega
            vt(jh)      =tide_harmonics(name(jh))%v0
            ut(jh)      =tide_harmonics(name(jh))%u
            ft(jh)      =tide_harmonics(name(jh))%f
         END DO
      ...
      IF( kt >= nit000_han .AND. kt <= nitend_han .AND. MOD(kt,nstep_han) == 0 ) THEN
         ...
         DO jh = 1, nb_ana
            DO jc = 1, 2
               ...
               DO_2D( 1, 1, 1, 1 )
                     ana_temp(ji,jj,nhc,1) = ana_temp(ji,jj,nhc,1) + ztemp * ssh(ji,jj,kmm) * ssmask (ji,jj) ! elevation
                     ana_temp(ji,jj,nhc,2) = ana_temp(ji,jj,nhc,2) + ztemp * uu_b(ji,jj,kmm) * ssumask(ji,jj) ! u-vel
                     ana_temp(ji,jj,nhc,3) = ana_temp(ji,jj,nhc,3) + ztemp * vv_b(ji,jj,kmm) * ssvmask(ji,jj) ! v-vel
               END_2D
            END DO
         END DO
      END IF
   ...
   SUBROUTINE dia_harm_end
      ...
      CALL lbc_lnk( 'dia_harm_end', out_eta, 'T', 1., out_u, 'U', -1. , out_v, 'V', -1. )
      !
